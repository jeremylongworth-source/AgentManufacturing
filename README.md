...
- [Latest handoff](docs/development/handoffs/AM-31-final-handoff.md): AM-31 assisted safety closure and the bounded AM-32 execution brief.
...
The [framework](docs/architecture/sector-specialization-framework.md) prioritizes the 12 roadmap groups and preserves 18 distinct labels in the [registry](specializations/registry.json). Sector requirements remain unimplemented. Inspect a planned coverage gap with `python scripts/inspect-sector-coverage.py automotive`; use `--generic-only` when no sector-dependent conclusion is requested.
...
python scripts/validate-sector-framework.py
