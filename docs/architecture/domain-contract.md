...
AM-09 operationalizes evidence checks in the [validation framework](validation-framework.md) and [framework contract](validation-framework-contract.json). These artifacts define structural, routing, deterministic-fixture, and evaluation layers while keeping future routes separate from observed behavior; they do not prove an unimplemented skill, authorize a host installation, or remove the AM-10 reference-skill gate.
