"""Validate AM-19 materials, BOM, and traceability packages."""
...
EXPECTED = {
    "validate-bill-of-materials": ("skills/family-12-materials-traceability/validate-bill-of-materials", "P1", "ROUTINE", "GENERIC_METHOD_ONLY", []),
    "calculate-material-requirement": ("skills/family-12-materials-traceability/calculate-material-requirement", "P1", "ROUTINE", "GENERIC_METHOD_ONLY", []),
    "analyze-material-variance": ("skills/family-12-materials-traceability/analyze-material-variance", "P1", "ROUTINE", "GENERIC_METHOD_ONLY", []),
    "trace-production-lot": ("skills/family-12-materials-traceability/trace-production-lot", "P1", "REGULATED", "PENDING_CONTEXT", ["SECTOR_REGULATED"]),
    "build-product-genealogy": ("skills/family-12-materials-traceability/build-product-genealogy", "P1", "ROUTINE", "GENERIC_METHOD_ONLY", []),
    "review-component-substitution": ("skills/family-12-materials-traceability/review-component-substitution", "P1", "ENGINEERING_OR_CERTIFICATION_BOUNDARY", "PENDING_CONTEXT", ["STANDARDS_DEPENDENT", "SECTOR_REGULATED"]),
    "reconcile-material-consumption": ("skills/family-12-materials-traceability/reconcile-material-consumption", "P1", "ROUTINE", "GENERIC_METHOD_ONLY", []),
    "identify-line-side-shortage": ("skills/family-12-materials-traceability/identify-line-side-shortage", "P1", "ROUTINE", "GENERIC_METHOD_ONLY", []),
    "review-scrap-material-record": ("skills/family-12-materials-traceability/review-scrap-material-record", "P2", "ROUTINE", "GENERIC_METHOD_ONLY", []),
}
...
def validate_package(root: Path, name: str, spec: tuple, records: dict[str, dict]) -> None:
    ...
    need(record is not None and record["family"] == "12", f"{name}: taxonomy family")
    ...
    for marker in ("12", "CORE", priority, safety, jurisdiction, "TAXONOMY_ACCEPTED_NOT_IMPLEMENTED") + tuple(flags):
...
def validate_evaluation(root: Path) -> None:
    content = (root / "tests/evaluations/AM-19-materials-traceability-acceptance.md").read_text(encoding="utf-8")
    for phrase in ("Status: `READY_FOR_REVIEW`", "Runtime model behavior", "NOT_RUN", "Residual risk"):
        need(phrase in content, f"AM-19 evaluation missing {phrase!r}")
    for name in EXPECTED:
        need(f"`{name}`" in content, f"AM-19 evaluation missing {name}")
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print("PASS: AM-19 materials/traceability; nine Family 12 packages, taxonomy traces, adapters, references, and acceptance evidence validated; runtime model behavior remains NOT_RUN.")
    return 0
