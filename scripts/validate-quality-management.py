"""Validate the AM-14 Family 06 quality-management and inspection packages."""
...
EXPECTED = {
    "review-quality-requirement": ("skills/family-06-quality-management/review-quality-requirement", "P1", "REGULATED", "PENDING_CONTEXT", ["STANDARDS_DEPENDENT", "SECTOR_REGULATED"]),
    "build-inspection-plan": ("skills/family-06-quality-management/build-inspection-plan", "P1", "REGULATED", "PENDING_CONTEXT", ["STANDARDS_DEPENDENT", "SECTOR_REGULATED"]),
    "review-product-conformity": ("skills/family-06-quality-management/review-product-conformity", "P1", "REGULATED", "PENDING_CONTEXT", ["STANDARDS_DEPENDENT", "SECTOR_REGULATED"]),
    "review-quality-record": ("skills/family-06-quality-management/review-quality-record", "P1", "ROUTINE", "GENERIC_METHOD_ONLY", []),
    "prepare-quality-release-package": ("skills/family-06-quality-management/prepare-quality-release-package", "P1", "REGULATED", "PENDING_CONTEXT", ["STANDARDS_DEPENDENT", "SECTOR_REGULATED"]),
    "audit-quality-process": ("skills/family-06-quality-management/audit-quality-process", "P2", "REGULATED", "PENDING_CONTEXT", ["STANDARDS_DEPENDENT", "SECTOR_REGULATED"]),
    "analyze-quality-kpis": ("skills/family-06-quality-management/analyze-quality-kpis", "P2", "ROUTINE", "GENERIC_METHOD_ONLY", []),
}
...
def validate_package(root: Path, name: str, spec: tuple, records: dict[str, dict]) -> None:
    ...
    need(record is not None and record["family"] == "06", f"{name}: taxonomy family")
    ...
    for marker in ("06", "CORE", priority, safety, jurisdiction, "TAXONOMY_ACCEPTED_NOT_IMPLEMENTED") + tuple(flags):
...
def validate_evaluation(root: Path) -> None:
    content = (root / "tests/evaluations/AM-14-quality-management-acceptance.md").read_text(encoding="utf-8")
    for phrase in ("Status: `READY_FOR_REVIEW`", "Runtime model behavior", "NOT_RUN", "Residual risk"):
        need(phrase in content, f"AM-14 evaluation missing {phrase!r}")
    for name in EXPECTED:
        need(f"`{name}`" in content, f"AM-14 evaluation missing {name}")
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print("PASS: AM-14 quality management; seven Family 06 packages, taxonomy traces, adapters, references, and acceptance evidence validated; runtime model behavior remains NOT_RUN.")
    return 0
